function [flag, xs, us, Dh] = MPC_2d_path_following_ground_func(x0, y0, h0, psi0, guidance, heights, wind_profile_hat)
...
% wind_h = @(h) vel_at6m.*log(h/0.04572)/log(6.096/0.04572);
...
for i = 1:N
    ...
    Prob.subject_to(X(:,i+1) == X(:,i) + Ts* [xy_dot * cos(X(3,i)) + wx(X(5,i));
                                              xy_dot * sin(X(3,i)) + wy(X(5,i));
                                              U(1,i);
                                              -U(2,i);
                                              -z_dot]);
end
