...
function [flag, guidance] = GuidanceSolve(N, vel_info, psi_dot_m, psi_ddot_m, init_cond, psi_dot_now)
...
cost = lambda1*(x(1,end)^2 + x(2,end)^2);
...
for i = 1:N-1
    % x2~xN
    Prob.subject_to(x(:,i+1) == x(:,i) + dt*[(Vh*cos(x(3,i))+Vh*cos(x(3,i+1)))/2;
                                             (Vh*sin(x(3,i))+Vh*sin(x(3,i+1)))/2;
                                             u(i)]);
    % u2~uN
